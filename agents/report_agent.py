...
def get_inventory_status(region: Optional[str] = None) -> Dict[str, Any]:
    ...
    inventory_summary = df.groupby('category')['qty'].sum().to_dict() # {"Electronics": 15,"Grocery": 27}
    region_summary = df.groupby('region')['qty'].sum().to_dict() # {"North": 15, "South": 20, "West": 7}

    return {
        'total_items': len(df), # total_items for region
        'low_stock_count': len(low_stock),
        'low_stock_items': low_stock.to_dict('records'), # [ {"sku": "A001","name": "Umbrella","category": "Rainwear","region": "North","qty": 5, "reorder_threshold": 20 }]  
